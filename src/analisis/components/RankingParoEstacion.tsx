...
export const RankingParoEstacion = () => {
  ...
  const obtenerRanking = async () => {
    try {
      const response = await fetch(
        "http://localhost:3000/api/analisis/obtenerRankingParosEstacion",
      );
        ...
        <Table className="bg-dark">
          <TableHeader className="bg-gray-800/50">
            <TableRow className="border-gray-700 hover:bg-transparent">
              ...
              <TableHead className="text-left text-gray-300">Línea</TableHead>
              ...
              >
                ...
                <TableCell className="text-right text-white">
                  <div className="flex items-center justify-end gap-1 font-mono text-lg font-bold">
                    {convertirSegundos(row.segundos)}
                  </div>
                </TableCell>
              </TableRow>
            ))}
